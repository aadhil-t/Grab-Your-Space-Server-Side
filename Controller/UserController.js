    ...
    try {
      ...
      if (emailExist) {
        ...
        if (userData) {
          ...
          SendMail(userData.name, userData.email, userData._id, "user verification",);
...
const EditProfile = async(req,res)=>{
    try {
        ...
        if(editUser){
            return res.status(200).json({data:editUser, message:"Profile edited successfully"})
        }
        else{
...
const UpdateEdit = async(req,res)=>{
    try {
        ...
        let userData = await User.findOneAndUpdate(
            ...
            if(userData){
               return res.status(200).json({updated:true,data:userData,message:"updated successfully"})
            }
...
const SignupWithGoogle = async(req,res)=>{
    try {
        ...
        if(exist){
            ...
            const token = jwt.sign({userId:user._id},process.env.UserSecret,{expiresIn:"24hr"})
...
const UserOtpVerify = async(req,res)=>{
    try {
        ...
        if(otp == userData.otp){
            ...
            const token = jwt.sign({userId:userData._id},process.env.UserSecret,{expiresIn:'1h'})
            res.json(token);
        }
        else{
